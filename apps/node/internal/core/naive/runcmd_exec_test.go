...
func TestRunCmd_Executes(t *testing.T) {
	out, err := realRunCmd(context.Background(), "/bin/sh", "-c", "printf iceslab")
	if err != nil {
	...
	out, err = realRunCmd(context.Background(), "/bin/sh", "-c", "echo boom >&2; exit 3")
	if err == nil {
	...
	cancel()
	if _, err := realRunCmd(ctx, "/bin/sh", "-c", "sleep 30"); err == nil {
