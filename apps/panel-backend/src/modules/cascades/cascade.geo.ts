...
export const EgressTargetSchema = z.enum(['direct', 'block', 'link-out', 'direction']);
...
export const EgressRuleSchema = z
  .object({
     ...
     *  forced through, overriding whatever the client asked for. Tags identify a
     *  DIRECTION and survive its node pool changing, which is why the policy
     *  stores the tag rather than a node id. */
 ...
 *  for a fixed outbound, or `{ balancerTag }` when a pool serves that way out.
 *
 *  Keys are the EgressTarget names, plus `direction:<tag>` for every direction
 *  the caller can steer into. A rule naming a direction the caller did not
 *  supply (deleted since the policy was authored) resolves to nothing and is
 *  DROPPED rather than emitted pointing nowhere - a rule with no outbound is a
 *  config xray refuses, which would take the whole node down over one stale
 *  policy row. */
...
export function compileEntryGeoRules(
  ...
  for (const r of policy ?? []) {
    ...
    // Unknown target (a direction deleted after the policy was authored): skip.
    // Emitting a rule with no outbound would make xray refuse the entire config.
