...
export async function refreshGeoAndRepush(): Promise<{
  ...
  // A custom category .dat changed -> re-push the members that serve it.
  const nodes = await repushEgressCascades();
  return { rebuilt: true, changed: true, nodes };
}

/**
 * Emit `cascade.changed` for every enabled cascade MEMBER carrying an egress
 * policy, so it re-renders its split and re-fetches the current custom .dat.
 * Returns the affected node count.
 *
 * Per NODE, not per cascade: a position is a pool, and only the boxes with a
 * split can have a changed config — re-pushing whole cascades would churn nodes
 * that cannot be affected. (Filtered in JS rather than a Prisma Json-null WHERE
 * to sidestep the JsonNull/DbNull filter quirks.) The node agent dedupes an
 * identical push, so a re-emit for an unchanged standard-only policy is a no-op
 * on the node.
 */
