 ...
 * body is read. Redirects are followed (GitHub release URLs 302 to the CDN);
 * per-hop SSRF re-check is a hardening TODO acceptable for an admin-configured,
 * admin-trusted source in v1 (same trust note as recipes.ssrf).
 *
...
export const fetchDat: DatFetcher = async (url, cond) => {
  assertFetchableUrl(url); // throws on non-https / private / metadata host
