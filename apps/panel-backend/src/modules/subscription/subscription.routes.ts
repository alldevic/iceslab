...
/** What to render, and — where the format is `wgconf` — in which flavour. */
interface Delivery {
  format: Format;
  proto: 'amneziawg' | 'wireguard' | undefined;
}

/**
 * Resolve what the client gets. `?proto=` is part of the answer and not a
 * separate question, because the two halves come from the same rule.
 *
 * The query wins on both counts, as before. Where it is silent the SRR rule
 * that named the format also names the flavour — and it has to, because a wg
 * client sends a User-Agent and never a query string. Until it could, the rule
 * put every wg-family client on `wgconf` and the builder picked whichever
 * endpoint came first: a stock WireGuard client was handed an AmneziaWG file
 * and answered `Line unrecognized: 'Jc = 4'`.
 *
 * The rule's flavour applies only when the rule's own format is the one being
 * rendered. An `?format=wgconf` that overrode a `singbox` rule must not pick up
 * that rule's (absent) flavour, and an explicit `?format=` on a wg rule keeps
 * the flavour the rule chose — which is what the install links already do.
 */
async function resolveDelivery(
  ...
  const matched = await matchRuleForUserAgent(userAgent);
  const format: Format = query.format
    ? query.format
    : matched && isFormat(matched.format)
    ? matched.format
    : acceptHeader.toLowerCase().includes('application/json')
    ? 'json'
    : 'plain';
  const ruleProto =
    matched && matched.format === format && (matched.proto === 'amneziawg' || matched.proto === 'wireguard')
      ? matched.proto
      : undefined;
  return { format, proto: query.proto ?? ruleProto };
}
...
export async function subscriptionRoutes(app: FastifyInstance): Promise<void> {
  ...
  app.get(`${config.SUBSCRIPTION_PATH_PREFIX}/:token`, {
    ...
    const userAgent = typeof request.headers['user-agent'] === 'string'
      ...
      : null;
    const { format, proto } = await resolveDelivery(
      query,
    ...
    try {
      ...
      switch (format) {
        ...
        case 'wgconf': {
          ...
          if (!picked.ok) return wgMiss(reply, picked, { ...query, proto }, 'wg');
          ...
          const tunnelName = wgConfName(
            ...
            query.node,
            // `query.proto`, NOT the resolved flavour. This string is the
            // TUNNEL NAME, and a client that identifies a tunnel by its name
            // treats a new one as a new server rather than a replacement. A
            // rule that starts choosing the flavour must not rename what the
            // buyer already has; the link that names the flavour is also the
            // link that names the file, and those are unchanged.
            query.proto,
          ...
          return reply
            ...
            .send(
              buildWgQuickConf(filtered, query.node, proto, {
