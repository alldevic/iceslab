  ...
  profiles: {
    ...
    form: {
      ...
      cfg: {
        ...
        pqMldsaDesc:
          'Серверный seed — добавляет постквантовую подпись на REALITY-сертификат. Только для security=reality. Target должен быть сайтом с БОЛЬШИМИ TLS-записями рукопожатия: постквантовый сертификат подгоняется под их длину, и слишком маленький target убивает все соединения к профилю (нода пишет «padding: -N», клиент молча ретраится). Измерено на xray 26.3.27: www.cloudflare.com не работает, www.amazon.com работает.',
