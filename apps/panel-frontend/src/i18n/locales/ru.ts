...
export default {
  ...
  hostsPage: {
    ...
    deleteLast:
      'Это последний хост на {{node}}. Инбаунд там продолжит работать, но из него в подписку больше ничего не попадёт.',
    deleted: 'Хост удалён',
  ...
  cascadeCreate: {
    ...
    writesDisabled:
      'API пока принимает старую форму с хопами, поэтому «Создать» выключено, пока позиции и направления не приедут на бэк.',
  },
