...
export default {
  ...
  users: {
    ...
    form: {
      ...
      importToken: 'Импорт sub-токена',
      importTokenDesc: 'опционально - переиспользовать ссылку при миграции',
  ...
  cascades: {
    ...
    hideHopsLabel: 'Прятать выходные ноды из подписки',
    hideHopsDesc:
      'Вкл (по умолчанию): выходные ноды каскада доступны только через его «Auto». Выкл: они также видны в подписке как отдельные ноды для прямого подключения.',
    split: 'Geo-сплит (entry-хоп)',
    splitAddRule: 'Добавить правило',
    splitHint:
      'Совпавший трафик выходит direct / блокируется / уходит в туннель к s2, раньше дефолта. Пусто = без сплита. Бандл-категории (например category-ru, youtube) работают как есть; кастомные — через ext:-файл из geo-реестра.',
    splitTarget: 'цель',
    targetDirect: 'direct',
    targetTunnel: 'туннель (s2)',
    targetBlock: 'блок',
