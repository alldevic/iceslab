...
export default {
  ...
  nodes: {
    ...
    edit: {
      ...
      bridgeToggleDesc:
        'Каналы, которые обслуживает sing-box (TUIC, AnyTLS, ShadowTLS, Hysteria2), собственных правил маршрутизации не рендерят, поэтому каскад, сплит и egress-политики их сегодня не касаются: трафик выходит прямо с этой ноды. С этим флагом они отдают трафик локальному xray по петле, и всё перечисленное начинает к ним применяться. Работает только на ВХОДЕ каскада, и только когда направление ровно одно — иначе выбирать страну за покупателя было бы нечем.',
