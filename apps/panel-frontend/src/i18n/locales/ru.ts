...
export default {
  ...
  userDrawer: {
    ...
    configs: 'конфигов',
    nodes: 'нод',
    // Не «нод»: несколько строк могут ехать через один вход, а выходы каскада
    // клиент прямыми конфигами не получает вовсе.
    addresses: 'адресов',
  ...
  cascadeCreate: {
    ...
    needDirection: 'Задай каждому направлению страну и ноду, чтобы включить «Создать».',
    tooManyLinks: 'Линков {{n}}, потолок {{max}}. Убери вход или направление.',
    // Формы, которые редактор умеет рисовать, а хранилище пока не умеет держать.
    // Названы явно, а не свёрнуты молча: пул, сохранённый как первая нода,
    // оставил бы оператора уверенным в резерве, которого нет.
    unsupported: {
      pool: 'Несколько нод на одной позиции пока негде хранить. Оставьте по одной ноде на позицию и на направление.',
      transitsWithFan:
        'Транзиты вместе с несколькими направлениями пока негде хранить. Уберите транзитные позиции или оставьте одно направление.',
    },
  },
  ...
  cascadeEdit: {
    ...
    subAuto: 'auto-выход',
    subVia: '{{name}} через {{where}}',
    subTag: 'тег {{tag}} · обычный',
    subDirect: 'ещё и напрямую',
    subHintMany:
      'Сквад может сузить список: с доступом к одному направлению участник видит Auto и только его. Направление, под которым нет живой ноды, в подписку не попадает вовсе.',
    subHintOne:
      'Направление одно, поэтому Auto-запись и запись направления ведут в один и тот же выход. Скваду без политик достаётся только обычная пара.',
    legacyProtocol:
