...
export default {
  ...
  userDrawer: {
    ...
    wgDevices: 'Туннели WireGuard',
    wgDevicesHint:
      'Свой ключ и свой адрес на каждое устройство. Трафик считает сама нода, поэтому подделать его клиент не может, а лимит не обойти: устройство без ключа не подключится.',
    wgDevicesEmpty: 'Туннелей нет: они нарезаются при первом обращении к подписке.',
    wgDevicesError: 'Не удалось загрузить туннели.',
    wgDeviceNever: 'не подключался',
    wgDeviceRevoked: 'отозван',
    wgDeviceRevoke: 'Отозвать туннель: ключ перестанет работать на всех нодах',
    wgDeviceRevoked_toast: 'Туннель отозван',
    traffic: 'Трафик',
    expires: 'Истекает',
  ...
  nodeEdit: {
    inService: 'В работе',
    inServiceDesc:
      'Выключено — нода уходит из всех подписок, и опрос статуса прекращается. Агент продолжает работать, ничего не удаляется.',
    coresTitle: 'Ядра на этой ноде',
    coresHint: 'спрошено вживую; панель сличает с тем, что несёт сама',
    coresUnreachable: 'Нода не ответила, поэтому что на ней запущено — неизвестно:',
    coresEmpty: 'Нода ответила и не назвала ни одного ядра.',
    coresRunning: 'работает',
    coresStopped: 'не работает',
    coresVersionUnknown: 'версию не сообщает',
    coresPinned: 'панель несёт {{version}}',
    coresNotPinned: 'для этого ядра панель не несёт артефакта',
    coresDrift: 'расходится с панелью',
  ...
  nodes: {
    ...
    edit: {
      ...
      inServiceDesc:
        'Выключено — нода уходит из всех подписок, и опрос статуса прекращается. Агент продолжает работать, ничего не удаляется.',
      inServiceOffWarning:
        'Нода выведена из работы: её не отдаёт ни одна подписка, статус больше не опрашивается.',
      system: 'Система',
      bindings: 'Bindings',
      ...
      egressSectionDesc:
        'Какой трафик выходит с ноды каким путём и какие каналы правило может назвать. Правила компилируются под ноду при пуше конфига.',
      bridgeToggle: 'Направлять не-xray каналы через локальный xray',
      bridgeToggleDesc:
        'Каналы, которые обслуживает sing-box (TUIC, AnyTLS, ShadowTLS, Hysteria2), собственных правил маршрутизации не рендерят, а WireGuard и AmneziaWG — модули ядра, их трафик вообще не попадает ни в один процесс. Поэтому каскад, сплит и egress-политики сегодня не касаются ни тех, ни других: трафик выходит прямо с этой ноды. С этим флагом sing-box отдаёт трафик локальному xray по петле, а трафик wg-интерфейсов туда же заворачивает сама нода (TPROXY), и всё перечисленное начинает применяться. Работает только на ВХОДЕ каскада, и только когда направление ровно одно — иначе выбирать страну за покупателя было бы нечем. ICMP по-прежнему уходит напрямую: xray его не переносит, а молча мёртвый ping читается как сломанный туннель.',
      zapret2Toggle: 'Канал десинхронизации zapret2',
      zapret2ToggleDesc:
        'Поднять здесь стек ss-zapret2: локальный SOCKS-прокси, чей собственный egress десинхронизирован против DPI, — правило, направленное в него, достаёт заблокированные адреса. Стек должен быть развёрнут на ноде (ansible-роль).',
      zapret2Preset: 'Пресет стратегии',
      ...
      egressPolicyDesc:
        'Сначала совпадение, потом путь наружу. Порядок = приоритет: xray берёт первое подошедшее правило, а всё несовпавшее идёт обычным для ноды путём. Нет правил — нода маршрутизирует как сейчас.',
      egressRule: 'Правило',
      egressRuleAdd: 'Добавить правило',
      egressRuleRemove: 'Удалить правило',
      egressGeosite: 'Категории geosite',
      egressGeoip: 'Категории geoip',
      egressDomain: 'Домены',
      egressIp: 'IP / CIDR',
      egressPort: 'Порт',
      egressNetwork: 'Сеть',
      egressNetworkAny: 'любая',
      egressTarget: 'Путь наружу',
      egressTargetDirect: 'напрямую (IP этой ноды)',
      egressTargetBlock: 'блокировать',
      egressTargetWarp: 'WARP',
      egressTargetZapret2: 'zapret2',
      egressTargetMissing:
        'У этой ноды нет такого пути наружу — правило будет отброшено при пуше конфига.',
      egressTuneTitle: 'Самоподобранная стратегия (сообщает нода)',
      egressTuneMeta: '{{domain}} · {{protocol}} · прошло {{working}} из {{total}} стратегий',
  ...
  profiles: {
    ...
    form: {
      ...
      cfg: {
        ...
        pqMldsaDesc:
          'Серверный seed — добавляет постквантовую подпись на REALITY-сертификат. Только для security=reality. ВАЖЕН TARGET: REALITY подгоняется под длину его записи рукопожатия только если та больше 512 байт, а постквантовый сертификат делает свою запись ~3.6 КБ — поэтому target посередине убивает все соединения к профилю (нода пишет «padding: -N», клиент молча ретраится). Маленький target не ограничивает вообще. Измерено на xray 26.3.27: www.cloudflare.com (2043 байта) не работает, www.amazon.com (41) работает.',
        ...
        presharedKeyLabel: 'Preshared key на устройство',
        presharedKeyDescription:
          'Подмешивает в каждое рукопожатие дополнительный симметричный ключ. Включение обесценивает все уже выданные конфиги: клиент без ключа не пройдёт рукопожатие с сервером, у которого он есть, и ни одна сторона не скажет почему. После включения попросите покупателей перекачать конфиги.',
  ...
  cascadeCreate: {
    ...
    directionLabel: 'Имя в клиенте',
    directionLabelStale:
      'Это имя больше не совпадает с тем, куда направление выходит. Оно закреплено, поэтому за страной не следует — в этом и смысл закрепления, и в этом же способ начать говорить подписчику не то назначение.',
  ...
  cascadeEdit: {
    ...
    nameHint:
      'Переименование меняет имя, под которым подписчик видит строки этого каскада, — кроме тех, у кого имя задано своё.',
    autoLabel: 'Имя строки Auto в клиенте',
    autoLabelHint:
      'Пусто — имя выводится из имени каскада, и тогда переименование каскада переименует этот сервер каждому, у кого он есть.',
    renamedTitle_one: '{{count}} строка сменила имя',
    renamedTitle_few: '{{count}} строки сменили имя',
    renamedTitle_many: '{{count}} строк сменили имя',
    renamedTitle_other: '{{count}} строк сменили имя',
    renamedBody:
      'Клиент, опознающий сервер по имени, добавит новую строку и оставит старую — а она больше никуда не ведёт. Подписчику надо обновить подписку И удалить старый сервер. Чтобы так не выходило, задайте строке своё имя и меняйте его.',
    ...
    hourAgo: '{{n}} ч назад',
    dayAgo: '{{n}} д назад',
    // Geo-сплит задаётся ПО НОДЕ: позиция — это пул, и сплит принадлежит той
    // коробке, которая выпускает трафик, а не каскаду целиком.
    split: 'Geo-сплит (по ноде)',
  ...
  geo: {
    sources: 'Geo-источники',
    addSource: 'Добавить источник',
    defaultBadge: 'по умолчанию',
    categories: 'Кастом-категории',
    ...
    builtAt: 'Собрано {{when}}',
    catStat: '{{name}}: {{domains}} доменов, {{cidrs}} IP',
    missing: 'нет {{list}}',
    sourceErrors: 'ошибок источников: {{count}} — {{first}}',
    rebuilt: 'Geo-артефакты пересобраны',
    editSource: 'Редактировать источник',
    newSource: 'Добавить источник',
    name: 'Название',
    nameHint: 'Только буквы, цифры, . _ - (имя используется как ext:-тег роутинга).',
    nameInvalid: 'Допустимы только буквы, цифры, . _ и -.',
    geositeUrl: 'URL geosite.dat',
    geoipUrl: 'URL geoip.dat',
    refreshInterval: 'Интервал обновления (часы)',
    refreshIntervalHint: 'Как часто плановое обновление перепроверяет источник (условный GET/ETag — дёшево). По умолчанию 24.',
    moveUp: 'Выше приоритет',
    moveDown: 'Ниже приоритет',
    mirrorGeosite: 'зеркало: geosite',
    mirrorGeoip: 'зеркало: geoip',
    mirrorHint:
      'Клиенты берут ПОЛНУЮ базу из этого источника (первый включённый по порядку списка). Переставьте, чтобы сменить. Кастом-категории всё равно объединяют все указанные источники.',
    everyHours: 'каждые {{n}}ч',
    browseCategories: 'Просмотр категорий',
    categoriesOf: 'Категории в «{{name}}»',
    tabGeosite: 'geosite ({{n}})',
    tabGeoip: 'geoip ({{n}})',
    filterCategories: 'Фильтр категорий…',
    noCategories: 'Нет подходящих категорий.',
    previewHint: 'Кликните категорию, чтобы увидеть записи.',
    previewCount: 'показано {{shown}} из {{total}}',
    sourceUrlHint: 'Хотя бы один URL. Только https; loopback / приватные хосты отклоняются.',
    editCategory: 'Редактировать категорию',
    newCategory: 'Добавить категорию',
    domainRefs: 'geosite-ссылки (доменные категории)',
    ipRefs: 'geoip-ссылки (IP-категории)',
    manualDomains: 'ручные домены',
    manualIps: 'ручные IP / CIDR',
    excludeDomains: 'исключить домены',
    addRef: 'Добавить ссылку',
    refSource: 'источник',
    refCategory: 'категория (напр. youtube)',
    rebuildHint: 'После правок пересоберите, чтобы обновить артефакты',
    removeRef: 'Удалить ссылку',
  },
  ...
  metadata: {
    ...
    entryPool: 'Пул входов',
    entryPoolHint: 'Сколько узлов профиль отдаёт в одну подписку максимум. 0 — все.',
    ...
    localProxyAuthTitle: 'Пароль на локальный прокси',
    localProxyAuthHint:
      'Конфиг Xray JSON поднимает socks и http на 127.0.0.1 самого покупателя. Android не изолирует петлю между приложениями, поэтому дотянуться до них может любое установленное приложение и ходить через туннель \u2014 тратя лимит и выходя с этого адреса. Включено \u2014 оба слушателя требуют пароль, выведенный из токена подписки; соседнее приложение прочитать его из песочницы клиента не может. Выключено по умолчанию, потому что системный прокси Android паролей не носит: клиент, работающий системным прокси, может перестать работать. Перед включением проверить на устройстве.',
    wgDnsTitle: 'DNS для WireGuard',
    wgDnsHint:
      'Список резолверов через запятую. Он пишется строкой DNS в каждый выдаваемый wg-конфиг. Пусто — строки не будет, и при полном туннеле клиент останется на резолвере своей сети: домашний 192.168.1.1 уедет в туннель и умрёт там. Хендшейк при этом проходит, а имена не разрешаются.',
    fragmentHint:
    ...
    wgUnusedTitle: 'Показывать неиспользованные туннели WireGuard',
    wgUnusedHint:
      'Туннели нарезаются заранее — по одному на каждое устройство, которое разрешает лимит покупателя, и до первого подключения. Выключено — в списке устройств только туннели, у которых был хендшейк: устройство это то, чем пользуются. Включено — видны и пустые слоты; это то, что нужно, когда разбираете «импортировал конфиг, ничего не появилось».',
  },
  regions: {
    title: 'Регионы',
    description:
  ...
  validation: {
    ...
    xray: {
      ...
      visionNeedsRawOrXhttp:
        'Vision flow работает только на raw и xhttp; на {{network}} панель снимет его при сохранении. Поставь Flow = «(none)» или Network = «raw» / «xhttp».',
      trojanIgnoresFlow:
  ...
  delivery: {
    ...
    patternRedos:
      'Здесь квантификатор вложен в квантификатор (вроде (a+)+). V8 откатывается, поэтому короткий подобранный User-Agent заставит её считать очень долго — и на публичном пути подписки, и в этой вкладке. Упростите.',
    ...
    protoTitle: 'Какой WireGuard',
    protoHint:
      'Формат wgconf рисует два несовместимых файла. Стоковый wireguard-tools конфиг AmneziaWG отвергает целиком («Line unrecognized: \'Jc = 4\'») и следом сносит интерфейс, поэтому флейвор выбирается здесь: настоящий клиент шлёт User-Agent, а не строку запроса.',
    protoAuto: 'Какой найдётся первым',
    protoAutoHint: 'Первый wg-эндпоинт подписки. Верно там, где флейвор один, и подбрасывание монеты там, где их два.',
    protoAmneziawg: 'AmneziaWG',
    protoAmneziawgHint: 'С обфускацией. Приложения AmneziaVPN и AmneziaWG.',
    protoWireguard: 'WireGuard',
    protoWireguardHint: 'Без обфускации. Официальные клиенты WireGuard, WireSock, WG Tunnel, ядерный wg-quick.',
  ...
  profileForm: {
    ...
    wgImportantTitle: 'Что важно знать про чистый WireGuard:',
    wgImportant1:
      'Подойдёт любой штатный клиент - официальные приложения WireGuard, WireSock, ядерный wg-quick. AmneziaWG-совместимое приложение не нужно.',
    wgImportant2:
      'Хендшейк опознаётся как WireGuard. Там, где DPI режет или блокирует WireGuard, бери AmneziaWG - у этого протокола обфускации нет by design.',
    wgImportant3:
      'Порт выберешь на следующем шаге («Развернуть на нодах»). Не используй 51820 - известный WG-default, ISP его режут. Если на ноде уже есть AmneziaWG, портам и подсетям нельзя совпадать.',
    wgImportant4:
      'Выход всегда прямой. Egress ноды (zapret2 и WARP) живёт на стороне xray, а ядерный wg-туннель ходит мимо xray, так что wg-профиль на такой ноде их не использует (у AmneziaWG так же).',
  ...
  hostsManager: {
    ...
    addressOverrideLabel: 'Address override',
    addressOverridePlaceholder: 'cdn.example.com - пусто = node.address',
    portOverrideLabel: 'Port override',
    sniOverrideLabel: 'SNI override',
    fingerprintLabel: 'Fingerprint',
    fingerprintDefault: '- по умолчанию -',
    pathOverrideLabel: 'Path override',
    pathOverridePlaceholder: '/path для ws/xhttp',
    hostHeaderLabel: 'Host header',
    alpnLabel: 'ALPN',
    alpnDesc: 'эмитится в URI-формат там, где формат его несёт',
    securityLayerLabel: 'Security layer',
    securityLayerDesc: 'default = REALITY/TLS адаптера; tls = поверх TLS на CDN',
    securityLayerDefault: 'default (адаптер сам решает)',
    allowInsecureLabel: 'Allow insecure cert',
    disableForFormatsDesc: 'host пропускается при выдаче этих форматов подписки',
    cancel: 'Отмена',
  },
  testConnect: {
    ...
    caveat:
      'Проба идёт из сети контейнера панели. Проверяет DNS / firewall / TLS-хендшейк, но НЕ доступность у конечного клиента (его провайдер может резать).',
    probing: 'Проверяю все bindings × hosts…',
    failed: 'Проба не удалась',
    noBindings: 'У этого профиля нет включённых bindings - нечего проверять.',
    rerun: 'Ещё раз',
    sniTooltip: 'TLS SNI, который мы отправили',
    certTooltip:
      'CN субъекта сертификата пира - для REALITY это должен быть маскировочный сайт, а не ваш домен',
    tlsTooltip: 'Согласованная версия TLS - REALITY требует, чтобы dest говорил TLSv1.3',
  },
