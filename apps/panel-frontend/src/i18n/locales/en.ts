...
export default {
  ...
  nodeEdit: {
    ...
    coresEmpty: 'The node answered and reported no cores at all.',
    coresRunning: 'running',
    coresStopped: 'not running',
    coresVersionUnknown: 'version not reported',
    coresPinned: 'panel carries {{version}}',
    coresNotPinned: 'the panel carries no artefact for this one',
    coresDrift: 'differs from the panel',
  ...
  nodes: {
    ...
    edit: {
      ...
      egressPolicyDesc:
        'Match first, then choose the way out. Order is the precedence: xray takes the first rule that matches, and anything unmatched keeps the node default. No rules = the node routes as it does today.',
      egressRule: 'Rule',
      egressRuleAdd: 'Add rule',
      egressRuleRemove: 'Remove rule',
      egressGeosite: 'geosite categories',
      egressGeoip: 'geoip categories',
      egressDomain: 'Domains',
      egressIp: 'IPs / CIDRs',
      egressPort: 'Port',
      egressNetwork: 'Network',
      egressNetworkAny: 'any',
      egressTarget: 'Way out',
      egressTargetDirect: 'direct (this node IP)',
      egressTargetBlock: 'block',
      egressTargetWarp: 'WARP',
      egressTargetZapret2: 'zapret2',
      egressTargetMissing:
        'This node has no such way out, so the rule is dropped when the config is pushed.',
      egressTuneTitle: 'Self-tuned strategy (reported by the node)',
      egressTuneMeta: '{{domain}} · {{protocol}} · {{working}} of {{total}} strategies got through',
  ...
  profiles: {
    ...
    form: {
      ...
      cfg: {
        ...
        pqMldsaDesc:
          'Server seed adding a post-quantum signature to the REALITY cert. Only for security=reality. MIND THE TARGET: REALITY matches the target\'s handshake record length only when that record is over 512 bytes, and the post-quantum certificate makes its own record ~3.6 KB - so a target in between kills every connection to this profile (the node logs "padding: -N", the client just retries). Smaller targets are never constrained. Measured on xray 26.3.27: www.cloudflare.com (2043 bytes) fails, www.amazon.com (41 bytes) works.',
  ...
  delivery: {
    ...
    protoTitle: 'Which WireGuard',
    protoHint:
      'wgconf renders two incompatible files. Stock wireguard-tools refuses an AmneziaWG config outright ("Line unrecognized: \'Jc = 4\'") and then deletes the device, so the flavour has to be chosen here — a real client sends a User-Agent, never a query string.',
    protoAuto: 'First one available',
    protoAutoHint: 'Whichever wg endpoint the subscription lists first. Right where you serve one flavour, a coin toss where you serve both.',
    protoAmneziawg: 'AmneziaWG',
    protoAmneziawgHint: 'Obfuscated. AmneziaVPN and AmneziaWG apps.',
    protoWireguard: 'WireGuard',
    protoWireguardHint: 'Plain. Official WireGuard apps, WireSock, WG Tunnel, kernel wg-quick.',
    tryTitle: 'Try it now',
  ...
  hostsManager: {
    ...
    addressOverridePlaceholder: 'cdn.example.com - empty uses node.address',
    portOverrideLabel: 'Port override',
    sniOverrideLabel: 'SNI override',
    fingerprintLabel: 'Fingerprint',
    fingerprintDefault: '- default -',
    pathOverrideLabel: 'Path override',
    pathOverridePlaceholder: '/path for ws/xhttp',
    hostHeaderLabel: 'Host header',
    alpnLabel: 'ALPN',
    alpnDesc: 'emitted into the URI format where the format carries it',
    securityLayerLabel: 'Security layer',
    securityLayerDesc: 'default = the adapter\'s REALITY/TLS; tls = on top of TLS at a CDN',
    securityLayerDefault: 'default (the adapter decides)',
    allowInsecureLabel: 'Allow insecure cert',
    disableForFormatsDesc: 'the host is skipped when these subscription formats are served',
    cancel: 'Cancel',
  },
  testConnect: {
    title: 'Test connect',
    caveat:
      "Probe runs from the panel container's network. Validates DNS / firewall / TLS handshake - but NOT end-user reachability (their ISP may still block).",
    probing: 'Probing all bindings × hosts…',
    failed: 'Probe failed',
    noBindings: 'This profile has no enabled bindings - nothing to probe.',
    rerun: 'Re-run',
    sniTooltip: 'TLS SNI we sent',
    certTooltip:
      'Peer cert subject CN - for REALITY this should be the masquerade target, not your domain',
    tlsTooltip: 'Negotiated TLS version - REALITY needs the dest to speak TLSv1.3',
  },
